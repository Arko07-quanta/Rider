...
const initializeDatabase = async () => {
  try {
    console.log("🛠️ Starting Consolidated Database Seeding...");
    ...
    // 2. Seed Promotions
    const promotions = [
      { code: 'SAVE50', type: 'percentage', val: 50, min_fare: 10, max_discount: 5 },
      { code: 'FLAT10', type: 'fixed', val: 10, min_fare: 15, max_discount: null },
      { code: 'WELCOME20', type: 'percentage', val: 20, min_fare: 0, max_discount: null },
      { code: 'LOYALTY50', type: 'fixed', val: 50, min_fare: 0, max_discount: null }
    ];
    ...
    for (const p of promotions) {
      ...
      if (exists.rows.length === 0) {
        await pool.query(
          `INSERT INTO promotions (code, discount_type, value, usage_limit, min_fare_amount, max_discount_amount) 
           VALUES ($1, $2, $3, 1000, $4, $5)`,
          [p.code, p.type, p.val, p.min_fare, p.max_discount]
        );
        console.log(`🎁 Seeded Promotion: ${p.code}`);
      }
    }
    ...
    for (const user of fakeUsers) {
      let userId: number;
      const userExists = await pool.query("SELECT user_id FROM users WHERE email=$1", [user.email]);
      ...
      await pool.query("INSERT INTO wallets (user_id, balance) VALUES ($1, 100.00) ON CONFLICT DO NOTHING", [userId]);
    }

    console.log("🚀 Consolidated Seeding Complete.");
    process.exit(0);
  } catch (err) {
